 ...
 */
 ...
 /*
 This has been adopted from the Apache license above, and re-worked to fit our use case
 Owner: Spectra LLC
 */
...
resource "google_service_account" "vault-admin" {
  account_id   = "spectra-vault-admin"
  project      = "${var.project_id}"
  display_name = "Spectra Vault Admin"
}
...
resource "google_project_iam_binding" "storage_admin" {
  ...
  members  = ["serviceAccount:${google_service_account.vault-admin.email}"]
}

resource "google_project_iam_binding" "service_account_actor" {
  project = "${var.project_id}"
...
resource "google_project_iam_binding" "crypto" {
  project = "${var.project_id}"
...
#   service_account_id     = "${google_service_account.vault-admin.name}"
